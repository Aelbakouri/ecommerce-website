...
<head> 
	...
	<style>       
		...
		.btn-block:hover {
            background-color: #CCE5FF; /* change the direction of the change here */
            color: #777;
		...
		.old-price{ margin-top: 10px; margin-left: 1.5rem; font-size: 12px;  text-decoration: line-through;  color:grey;}
		.coppon{
			position: absolute;
			right: 22px;
			top: 12px;
...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>catégories de produits</h4></a></li>
	";
...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			$cop = $pro_price/$old_price;
			$coppon = 1-$cop;
			$cn= floor($coppon*100);

			echo "
			<div class='col-md-3'>
			<div class='panel panel-info disp-block p-2' id='test'>
				<span class='coppon'>-$cn%</span>
				<div class='panel-body'>
					<img src='product_images/$pro_image' style='width:220px; height:250px;'/>
				</div>
				<div class='panel1'><strong>$pro_title</strong> </div>
				<div class='panel1'><span class='price'>$pro_price ".CURRENCY."</span><br>  <span class='old-price'>$old_price Dh</span></div>
				<button pid='$pro_id' type='button' id='product' class='btn btn-primary btn-l btn-block'>Ajouter au panier</button>
				</div>
			</div>
		</div>
	...
	if(isset($_POST["addToCart"])){
		...
		if($count > 0){
			...
			<script>Swal.fire({
				position: 'top-end',
				icon: 'warning',
				title: 'Le produit est déjà ajouté au panier Continuez vos achats..!',
			...
			if (mysqli_num_rows($query) > 0) {
				...
				<script>Swal.fire({
					position: 'top-end',
					icon: 'warning',
					title: 'Le produit est déjà ajouté au panier Continuer mes achats..!',
					showConfirmButton: false,
					timer: 2300
				  }
				  )</script>
					...
					// <div class='alert alert-warning'>
					// 		<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
					// 		<b>Le produit est déjà ajouté au panier Continuer mes achats..!</b>
					// </div>
					exit();
			}
			$sql = "INSERT INTO `cart`
			...
			if (mysqli_query($con,$sql)) {
				...
				// <div class='alert alert-success'>
				// 		<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				// 		<b>Votre produit a été ajouté au panier!</b>
				// 	</div>
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Prêt à passer à la caisse" >
							</form>';
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		...
		Swal.fire({
				position: 'top-end',
				icon: 'error',
				title: 'Le produit est retiré du panier',
				showConfirmButton: false,
				timer: 3000
			  }
			  )</script>
		...
		// 				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times</a>
		// 				<b>Le produit est retiré du panier</b>
		// 		</div>
		exit();
	}
}
